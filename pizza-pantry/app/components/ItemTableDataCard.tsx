...
const ItemTableDataCard :React.FC<Partial<ItemTableDataCardProps>> = ({
    obj,select,editModalID,deleteModalID
}) => {
  ...
  return (
    <>
        ...
        <td className="text-center space-x-2">
            <button 
                onClick={()=>{if(select && obj){ select(obj); console.log(obj); }}}>
            <label htmlFor={ editModalID? editModalID : defaultModalId}
            ...
            <button  onClick={()=>{if(select && obj){ select(obj); console.log(obj); }}}className="btn btn-sm btn-ghost tooltip text-error" data-tip="Delete Item">
                    <label htmlFor={ deleteModalID?deleteModalID: defaultModalId}>
